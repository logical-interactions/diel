...
export function DistributeQueries(rt: DielRuntime): DielPhysicalExecution {
  ...
  const justShared = ir.justShared;
  ...
  //examining each worker
  //note: outputs are derivedRelations, inputs are originalRelations
  workerDependency.forEach((wDep, keyW) => {
    ...
    ir.dependencies.inputDependenciesOutput.forEach((iDep, keyI) => {
      ...
      if (intersect.size > 0) {
        ...
        const inputOnMain = ir.allOriginalRelations.get(keyI);
        if(!justShared.has(inputOnMain)) {
          workers.get(wLoc).originalRelations = workers.get(wLoc).originalRelations.concat(inputOnMain);
          ir.justShared.add(inputOnMain);
